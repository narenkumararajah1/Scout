...
from backend.database.models import BusinessInitiative, Executive, Technology
...
from backend.repositories.postgres.technology_repository import list_technologies_for_company, upsert_technology
...
async def persist_extracted_entities(company_id: str, extracted: ExtractedEntities) -> tuple:
    """Persists Knowledge Extraction's technologies/business initiatives/
    executives for one company. Idempotent: technologies and business
    initiatives upsert by (company_id, name); executives check for an
    existing name first and update rather than duplicate, since the same
    company is re-researched across cycles.
    """
    technologies = [
